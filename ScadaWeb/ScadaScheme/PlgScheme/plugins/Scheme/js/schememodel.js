...
scada.scheme.Scheme.prototype._continueLoading = function (viewOrEditorID, callback) {
  const getCurTime = scada.utils.getCurTime
...
scada.scheme.Scheme.prototype._loadPart = function (viewOrEditorID, callback) {
  const LoadStates = scada.scheme.LoadStates
...
scada.scheme.Scheme.prototype._loadSchemeDoc = function (viewOrEditorID, callback) {
  const operation = this.serviceUrl + 'GetSchemeDoc'
  const thisScheme = this
...
scada.scheme.Scheme.prototype._loadComponents = function (viewOrEditorID, callback) {
  const operation = this.serviceUrl + 'GetComponents'
  const thisScheme = this
...
scada.scheme.Scheme.prototype._appendComponents = function (parsedComponents) {
  for (const parsedComponent of parsedComponents) {
    ...
    const schemeComponent = new scada.scheme.Component(parsedComponent)
...
scada.scheme.Scheme.prototype._loadImages = function (viewOrEditorID, callback) {
  const operation = this.serviceUrl + 'GetImages'
  const thisScheme = this
...
scada.scheme.Scheme.prototype._loadErrors = function (viewOrEditorID, callback) {
  const operation = this.serviceUrl + 'GetLoadErrors'
  const thisScheme = this
...
scada.scheme.Scheme.prototype.createDom = function (opt_controlRight) {
  ...
  for (const component of this.componentMap.values()) {
    try {
      ...
      if (this.dom) {
        const elem = this.renderContext.editMode ? component.renderer.wrap(component) : component.dom
...
scada.scheme.Scheme.prototype.updateData = function (clientAPI, callback) {
  ...
  clientAPI.getCurCnlDataExt(this._cnlFilter, function (success, cnlDataExtArr) {
    if (success) {
      const curCnlDataMap = clientAPI.createCnlDataExtMap(cnlDataExtArr)

      if (curCnlDataMap) {
        thisScheme.renderContext.curCnlDataMap = curCnlDataMap

        for (const component of thisScheme.componentMap.values()) {
...
scada.scheme.Scheme.prototype.setScale = function (scale) {
  try {
    const scaleVal = $.isNumeric(scale) ? scale : this.renderer.calcScale(this, scale)
...
scada.scheme.Scheme.prototype.sendCommand = function (ctrlCnlNum, cmdVal, viewID, componentID, callback) {
  const operation = this.serviceUrl + 'SendCommand'
  const thisScheme = this

  const frameID = this._openCommandFrame()
  const closeTimeoutID = setTimeout(this._closeCommandFrame.bind(this), this.CLOSE_CMD_TIMEOUT, frameID)
