...
scada.scheme.Renderer.prototype.setBackColor = function (jqObj, backColor, opt_removeIfEmpty, opt_statusColor) {
  if (backColor) {
    if (backColor == this.STATUS_COLOR) {
      jqObj.css('background-color', opt_statusColor || this.STATUS_DISPLAY_COLOR)
    } else {
...
scada.scheme.Renderer.prototype.setBorderColor = function (jqObj, borderColor, opt_removeIfEmpty, opt_statusColor) {
  if (borderColor) {
    if (borderColor == this.STATUS_COLOR) {
      jqObj.css('border-color', opt_statusColor || this.STATUS_DISPLAY_COLOR)
    } else {
...
scada.scheme.Renderer.prototype.setForeColor = function (jqObj, foreColor, opt_removeIfEmpty, opt_statusColor) {
  if (foreColor) {
    if (foreColor == this.STATUS_COLOR) {
      jqObj.css('color', opt_statusColor || this.STATUS_DISPLAY_COLOR)
    } else {
...
scada.scheme.SchemeRenderer.prototype.createDom = function (component, renderContext) {
  const divScheme = $("<div class='scheme'></div>")
...
scada.scheme.SchemeRenderer.prototype.refreshImages = function (component, renderContext, imageNames) {
  if (component.dom) {
    ...
    if (Array.isArray(imageNames) && imageNames.includes(props.BackImageName)) {
      const divSchemeBack = component.dom.find('.scheme-back:first')
      const backImage = renderContext.getImage(props.BackImageName)
...
scada.scheme.SchemeRenderer.prototype.updateDom = function (component, renderContext) {
  if (component.dom) {
    const props = component.props // scheme document properties
    const schemeWidth = props.Size.Width
    const schemeHeight = props.Size.Height

    const divScheme = component.dom.first()
    divScheme.css({
    ...
    let divSchemeBack = divScheme.children('.scheme-back:first')
...
scada.scheme.SchemeRenderer.prototype.calcScale = function (component, scaleStr) {
  if (component.parentDomElem) {
    const Scales = scada.scheme.Scales
    const areaWidth = component.parentDomElem.innerWidth()
    const schemeWidth = component.props.Size.Width
    const horScale = areaWidth / schemeWidth

    if (scaleStr == Scales.FIT_SCREEN) {
      const schemeHeight = component.props.Size.Height
      const areaHeight = component.parentDomElem.innerHeight()
      const vertScale = areaHeight / schemeHeight
...
scada.scheme.SchemeRenderer.prototype.setScale = function (component, scaleVal) {
  if (component.dom) {
    const sizeCoef = Math.min(scaleVal, 1)
    component.dom.css({
...
scada.scheme.ComponentRenderer.prototype._getDynamicColor = function (color, cnlNum, renderContext) {
  if (color) {
    if (color == this.STATUS_COLOR) {
      const curCnlDataExt = renderContext.curCnlDataMap
...
scada.scheme.ComponentRenderer.prototype.setHAlign = function (jqObj, hAlign) {
  const HorizontalAlignments = scada.scheme.HorizontalAlignments
...
scada.scheme.ComponentRenderer.prototype.setVAlign = function (jqObj, vAlign) {
  const VerticalAlignments = scada.scheme.VerticalAlignments
...
scada.scheme.ComponentRenderer.prototype.bindAction = function (jqObj, component, renderContext) {
  const Actions = scada.scheme.Actions
  const props = component.props
  ...
  if (actionIsBound) {
    ...
    if (!renderContext.editMode) {
      const viewHub = renderContext.schemeEnv.viewHub
      ...
      jqObj.click(function () {
        switch (props.Action) {
          case Actions.DRAW_DIAGRAM:
            if (dialogs) {
              const date = viewHub.curViewDateMs ? new Date(viewHub.curViewDateMs) : new Date()
...
scada.scheme.ComponentRenderer.prototype.setLocation = function (component, x, y) {
  ...
  if (component.dom) {
    const compParent = component.dom.parent()
...
scada.scheme.ComponentRenderer.prototype.setWrapperProps = function (component) {
  // set location
  const location = this.getLocation(component)
  this.setLocation(component, location.x, location.y)

  // set font
  if (component.props && component.dom) {
    const wrapper = component.dom.parent('.comp-wrapper')
...
scada.scheme.ComponentRenderer.prototype.wrap = function (component) {
  const compWrapper = $("<div class='comp-wrapper'></div>").append(component.dom)
...
scada.scheme.StaticTextRenderer.prototype.createDom = function (component, renderContext) {
  ...
  const spanComp = $("<span id='comp" + component.id + "'></span>")
  ...
  if (props.AutoSize) {
    ...
    const borders = props.BorderWidth * 2
...
scada.scheme.StaticTextRenderer.prototype.setSize = function (component, width, height) {
  ...
  const spanText = component.dom.children()
  spanText.css({
    'max-width': width,
    width: width,
    height: height
...
scada.scheme.DynamicTextRenderer.prototype.createDom = function (component, renderContext) {
  ...
  const ShowValueKinds = scada.scheme.ShowValueKinds
  const props = component.props
  const spanComp = component.dom.first()
  const spanText = component.dom.children()
  const cnlNum = props.InCnlNum
...
scada.scheme.DynamicTextRenderer.prototype.updateData = function (component, renderContext) {
  ...
  if (props.InCnlNum > 0) {
    const ShowValueKinds = scada.scheme.ShowValueKinds
    const spanComp = component.dom
    const spanText = spanComp.children()
    ...
    const isHovered = spanComp.is(':hover')

    const backColor = this.chooseColor(isHovered, props.BackColor, props.BackColorOnHover)
    const borderColor = this.chooseColor(isHovered, props.BorderColor, props.BorderColorOnHover)
    const foreColor = this.chooseColor(isHovered, props.ForeColor, props.ForeColorOnHover)
...
scada.scheme.StaticPictureRenderer.prototype.createDom = function (component, renderContext) {
  const ImageStretches = scada.scheme.ImageStretches
  const props = component.props

  const divComp = $("<div id='comp" + component.id + "'></div>")
...
scada.scheme.DynamicPictureRenderer.prototype.createDom = function (component, renderContext) {
  ...
  divComp.hover(
    function () {
      ...
      if (cnlNum <= 0) {
        const image = renderContext.getImage(props.ImageOnHoverName)
...
scada.scheme.DynamicPictureRenderer.prototype.updateData = function (component, renderContext) {
  ...
  if (props.InCnlNum > 0) {
    ...
    // set the image
    const image = renderContext.imageMap.get(imageName)
    this.setBackgroundImage(divComp, image, true)

    // choose and set colors of the component
    const statusColor = cnlDataExt.Color
    const isHovered = divComp.is(':hover')
...
scada.scheme.RenderContext.prototype.getCnlDataExt = function (cnlNum) {
  ...
  if (!curCnlDataExt) {
    ...
    const renderer = new scada.scheme.Renderer()
