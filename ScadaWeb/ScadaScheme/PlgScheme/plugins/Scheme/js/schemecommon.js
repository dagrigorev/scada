...
scada.scheme.calc = {
  ...
  compare: function (val1, val2, compareOper) {
    const CompareOperators = scada.scheme.CompareOperators
  ...
  isTrue: function (val1, val2, logicalOper) {
    const LogicalOperators = scada.scheme.LogicalOperators
  ...
  conditionSatisfied: function (cond, cnlVal) {
    const LogicalOperators = scada.scheme.LogicalOperators
    const comp1 = this.compare(cnlVal, cond.CompareArgument1, cond.CompareOperator1)

    if (cond.LogicalOperator == LogicalOperators.NONE) {
      return comp1
    } else {
      const comp2 = this.compare(cnlVal, cond.CompareArgument2, cond.CompareOperator2)
