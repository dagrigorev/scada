var scada = scada || {}
const viewHub = new scada.ViewHub(window)
...
scada.view = {
  ...
  _dataWindow: {
    ...
    reload: function () {
      if (this.url) {
        const viewID = viewHub.curViewID
        const newUrl = this.dependsOnView && viewID > 0
          ? scada.utils.setQueryParam('viewID', viewID, this.url)
          : this.url
        const frameDataWindow = scada.utils.setFrameSrc($('#frameDataWindow'), newUrl)
        frameDataWindow
          .off('load')
          .on('load', function () {
  ...
  _loadSplitterPosition: function () {
    let dataWindowHeight = parseInt(localStorage.getItem('Shell.DataWindowHeight')) ||
            $('#divDataWindow').outerHeight()
    const minHeight = parseInt($('#divDataWindow').css('min-height'), 10)
    const maxHeight = $('#divViewContent').innerHeight() -
  ...
  _loadActiveDataWindow: function () {
    const activeDataWindow = localStorage.getItem('Shell.ActiveDataWindow')
    const thisView = this
  ...
  updateLayout: function () {
    const divViewContent = $('#divViewContent')
    const divViewSplitter = $('#divViewSplitter')
    const divDataWindow = $('#divDataWindow')
    const divBottomTabs = $('#divBottomTabs')

    const totalH = divViewContent.innerHeight()
    const splitterH = this._getOuterHeight(divViewSplitter)
    const dataWindowH = this._getOuterHeight(divDataWindow)
    const bottomTabsH = this._getOuterHeight(divBottomTabs)
  ...
  activateDataWindow: function (divClickedTab) {
    const url = divClickedTab.data('url')
    if (url) {
  ...
  loadView: function (viewID, viewUrl, opt_from_history) {
    ...
    const frameView = scada.utils.setFrameSrc($('#frameView'), viewUrl)

    frameView
      .off('load')
      .on('load', function () {
        const wnd = frameView[0].contentWindow
  ...
  saveSplitterPosition: function () {
    const dataWindowHeight = $('#divDataWindow').outerHeight()
