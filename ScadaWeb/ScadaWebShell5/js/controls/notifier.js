...
scada.Notifier.prototype.addNotification = function (messageHtml, notifType, lifetime) {
  ...
  const divPrevMessage = this._notifier.children('.message:last')
  ...
  // add the new message
  const divMessage = $("<div class='message'></div>").html(messageHtml)
...
scada.Notifier.prototype.clearOutdatedNotifications = function () {
  const messages = this._notifier.find('.message')
