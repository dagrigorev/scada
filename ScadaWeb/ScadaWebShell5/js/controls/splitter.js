...
scada.SplitterBulk.prototype._getMinWidth = function (jqObj) {
  const minWidth = parseInt(jqObj.css('min-width'), 10)
  return minWidth || this.minSize
}

// Get minimum height of the resized div
scada.SplitterBulk.prototype._getMinHeight = function (jqObj) {
  const minHeight = parseInt(jqObj.css('min-height'), 10)
  return minHeight || this.minSize
}
...
scada.SplitterBulk.prototype._addOverlay = function () {
  const overlay = $("<div class='splitter-overlay'><div/>").css({
...
scada.SplitterBulk.prototype._changePosition = function (x, y) {
  if (this._isResizing) {
    ...
    if (this.isHorizontal) {
      const nextDivCurH = this.nextDiv.height()
      const prevDivCurH = this.prevDiv.height()
      const deltaY = y - this._curY
      var delta = deltaY >= 0
      ...
      const nextDivCurW = this.nextDiv.width()
      const prevDivCurW = this.prevDiv.width()
      const deltaX = x - this._curX
      var delta = deltaX >= 0
...
scada.splitter = {
  ...
  _createBulks: function () {
    ...
    $('.splitter').each(function () {
      const bulk = new scada.SplitterBulk()
      bulk.splitter = $(this)
  ...
  _stopAllResizing: function () {
    for (const bulk of this._splitterBulks) {
