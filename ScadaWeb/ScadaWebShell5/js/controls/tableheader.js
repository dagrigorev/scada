...
scada.tableHeader = {
    ...
    fixedHeader.find('td').each(function (index) {
      ...
      if (origCell.css('display') != 'none') {
        ...
        if (origCellSpan.length > 0) {
          const cellWidth = origCellSpan[0].getBoundingClientRect().width // fractional value is required
  ...
  create: function () {
    ...
    $('.table-wrapper').each(function () {
      ...
      const setHeaderTopFunc = function () {
  ...
  update: function () {
    ...
    $('.table-wrapper').each(function () {
      const wrapper = $(this)
      const table = wrapper.children('table')
      const fixedHeader = table.find('tr.fixed-table-header:first')

      if (fixedHeader.length > 0 /* already created */) {
        const origHeader = table.find('tr.orig-table-header:first')
        thisObj._updateHeaderCellWidths(origHeader, fixedHeader)
