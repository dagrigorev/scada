...
scada.Popup.prototype._getModalButtonCaption = function (btn) {
  const btnCaption = scada.modalButtonCaptions ? scada.modalButtonCaptions[btn] : null
  return btnCaption || btn
}
...
scada.Popup.prototype.showModal = function (url, opt_options, opt_callback) {
  ...
  // create the frame
  const modalFrame = $("<iframe class='modal-frame'></iframe>").css({
    position: 'fixed',
  ...
  modalFrame
    ...
    .one('load', function () {
      ...
      const specifiedHeight = opt_options ? opt_options.height : 0

      // tune the modal
      const frameWnd = modalFrame[0].contentWindow
      const frameAccessible = scada.utils.checkAccessToFrame(frameWnd)
...
scada.Popup.prototype.setModalResult = function (modalWnd, dialogResult, extraParams) {
  const modalElem = $(modalWnd.frameElement).closest('.modal')
  modalElem
