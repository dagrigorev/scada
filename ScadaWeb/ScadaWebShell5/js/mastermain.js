var scada = scada || {}
const popup = new scada.Popup()
const ajaxQueue = new scada.AjaxQueue(scada.env.rootPath)
...
scada.masterMain = {
  ...
  updateLayout: function () {
    const divMainHeader = $('#divMainHeader')
    const divMainLeftPane = $('#divMainLeftPane')
    const divMainTabs = $('#divMainTabs')

    const headerH = divMainHeader.css('display') == 'none' ? 0 : divMainHeader.outerHeight()
    const paneH = $(window).height() - headerH
  ...
  chooseToolWindow: function () {
    const explorerVisible = scada.env.rootPath + 'View.aspx' == window.location.pathname
  ...
  loadVisualState: function () {
    const leftPaneVisible = localStorage.getItem('Shell.LeftPaneVisible')
