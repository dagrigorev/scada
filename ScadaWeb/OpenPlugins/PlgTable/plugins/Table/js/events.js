...
const STOP_SCROLL_PERIOD = 30000

// Filter events by view
let eventsByView = true
...
function loadEventFilter () {
  const val = localStorage.getItem('Table.EventsByView')
...
function exportEvents () {
  const exportUrl = 'EventsRepOut.aspx?viewID=' + (eventsByView ? viewID : '') +
...
function appendEvents (tableElem, eventArr, startIndex) {
  const len = eventArr.length ? eventArr.length : 0
  let beep = false
...
function afterLoading () {
  ...
  if ($('#tblEvents tr.event:first').length > 0) {
    const divTblWrapper = $('#divTblWrapper')
    ...
    if (initialLoad || (new Date()) - activityTime > STOP_SCROLL_PERIOD) {
      const scrollHeight = divTblWrapper[0].scrollHeight
      const newScrollTop = scrollHeight - divTblWrapper.innerHeight()
...
function updateEvents (full, callback) {
  const reqViewDate = viewDate
  const reqCnlFilter = cnlFilter
  const startEvNum = full ? 0 : lastEvNum + 1
  const reqDataAge = full ? fullDataAge : partialDataAge
  ...
  scada.clientAPI.getEvents(reqViewDate, reqCnlFilter, dispEventCnt, startEvNum, reqDataAge,
    function (success, eventArr, dataAge) {
      if (reqViewDate != viewDate || reqCnlFilter != cnlFilter) {
        ...
        const tableElem = $('#tblEvents')
        const eventArrLen = eventArr.length ? eventArr.length : 0

        if (full) {
          if (eventArrLen > 0) {
            const firstEvNum = eventArr[0].Num
            let firstEvInd = 0
            const eventRowsCnt = eventRows.length
            while (firstEvInd < eventRowsCnt && eventRows[firstEvInd].data('num') < firstEvNum) {
              firstEvInd++
            }

            const eventsToMerge = eventRowsCnt - firstEvInd
            let evNumsMatched = eventsToMerge <= eventArrLen
            let eventRowInd = firstEvInd
            let eventArrInd = 0
