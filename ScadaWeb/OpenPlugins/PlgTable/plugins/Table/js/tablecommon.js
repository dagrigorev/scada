...
const VIEW_DATE_OPTIONS = { year: 'numeric', month: 'long', day: '2-digit' }
// Error message that is shown if dialogs object is undefined
const DIALOGS_UNDEFINED = 'Dialogs object is undefined'

// The view hub object
const viewHub = scada.viewHubLocator.getViewHub()
// Notifier control
const notifier = null
// Current view date
let viewDate = null
...
function updateLayout () {
  ...
  const toolbarHeight = divToolbar.outerHeight()
...
function setViewDate (dateMs) {
  ...
  const viewDateStr = viewDate.toLocaleDateString(locale, VIEW_DATE_OPTIONS).replace(/\u200E/g, '')
  // display date
  ...
  if (dialogs) {
    ...
    dialogs.showCalendar(txtDate, txtDate.val(), function (dialogResult, extraParams) {
      if (dialogResult) {
        ...
        const date = new Date(extraParams.date.getTime())
